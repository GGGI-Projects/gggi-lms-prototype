...
/**
 * The Law library's own list, searchable - same shape as `<MaterialShelf>`,
 * built here rather than server-rendered for the same reason: a row is a
 * record (title, reference, tags, status), not a join across several other
 * records, so there is nothing saved by rendering it on the server first.
 *
 * FILTERS BY STATUS AND BY TAG, in place of the Materials shelf's group and
 * kind - the two questions asked of this library are "what still needs
 * publishing" and "is there anything about flooding".
 */
...
export function LawShelf({ entries }: { entries: LawShelfEntry[] }) {
  ...
  const shown = useMemo(() => {
    ...
    return entries.filter((entry) => {
      const matches =
        !needle ||
        `${entry.title} ${entry.summary} ${entry.reference}`
          .toLowerCase()
          .includes(needle);
      return (
        matches &&
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <label className="relative block w-full lg:max-w-sm">
          <span className="sr-only">Search laws</span>
          ...
          {shown.length ? (
            shown.map((entry) => (
              <li key={entry.id} className="px-5 py-4">
                <Link href={`/laws-admin/${entry.id}`} className="block">
      ...
      <p className={`mt-4 ${META.base}`}>
        {shown.length} of {entries.length} laws shown
      </p>
    </div>
  );
}
