...
import { quizNeedsAttention, type ConsoleModule } from "@/lib/admin";
...
import { AttachModuleMaterials } from "@/components/console/attach-materials-action";
import {
 ...
 * Managing a module.
 *
 * ONE COMPONENT FOR TWO CONSOLES. An administrator opening a module and the
 ...
 *
 *   1. CONTENT - the lecture and the argument, shown close to how a learner
 ...
 *      exists: the unit cost table is one file used by four modules, not four
...
export function ModuleEditor({
  ...
  mod: ConsoleModule;
  /** The written module, or null for one that is still only a plan. */
  content: Module | null;
      ...
      <div className="min-w-0 space-y-10 lg:col-span-2">
        ...
        {content ? (
          <EditorSection
            index={1}
            title="Content"
            description="In the order a learner meets it. Each block is a separate thing to write, and their order is the module's argument."
          >
          ...
          <EditorSection
            index={1}
            title="Content"
            description="Nothing has been written for this module yet."
          >
            <div className="rounded-sm border border-dashed border-muted-light bg-paper-raised px-8 py-14 text-center">
              ...
              <p className={`measure mx-auto mt-3 ${BODY.base}`}>
                A module is a short list of blocks: a lecture or a piece of
        ...
        <EditorSection
          ...
          description="What the learner downloads. Everything here comes off the shared library, so one file can serve six modules and be replaced once."
                ...
                <li key={item.title} className="flex items-center gap-4 px-5 py-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={`mt-0.5 truncate ${META.base}`}>
                      ...
                      {item.entry
                        ? ` · ${item.entry.group?.name ?? "no group"} · used by ${item.entry.usage.length} ${item.entry.usage.length === 1 ? "module" : "modules"}`
                        : " · uploaded straight onto this module"}
                    </p>
                  </div>
        ...
        >
          {quiz.questions ? (
            ...
            <p className={`rounded-sm border border-dashed border-muted-light bg-paper-raised px-6 py-10 text-center ${BODY.base}`}>
              A quiz is written once the module has content.
            </p>
          )}
          ...
          {quizNeedsAttention(quiz.stats) && quiz.stats ? (
            <div className="mt-4">
              <Callout title="This quiz is sending people back">
                ...
                attempts. When the modules either side of one are fine, it is
      ...
      <aside className="min-w-0 space-y-4">
        <Panel>
          ...
          <p className={`mt-2 ${BODY.base}`}>
            Only published modules appear in a learner&rsquo;s contents list.
          </p>
        ...
        <Panel>
          <h2 className="font-display text-2xl tracking-tight text-ink">Module</h2>
          <DefinitionList
            className="mt-5"
            items={[
              {
              ...
              { term: "Position", value: `Module ${mod.number}` },
              {
                term: "State",
                value: (
        ...
        <Panel>
          ...
          <p className={`mt-2 ${BODY.base}`}>
            Deleting a module renumbers the ones after it and removes it from
            the progress of everybody part-way through the programme. Files it
 ...
 * shape of a module - lecture, then argument - is readable down the left edge
...
/** The banner a module inside an unpublished programme carries. */
export function DraftModuleNotice({ programme }: { programme: string }) {
  return (
    <Callout tone="info" title="Not published">
      Nothing on this page is visible to a learner. {programme} stays hidden
      from the catalogue until its modules are written and published.
    </Callout>
  );
}
