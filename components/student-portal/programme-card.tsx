...
import { formatDuration, type ProgrammeProgress } from "@/lib/portal";
import { BODY, META } from "@/lib/theme";

/**
 * One programme, as a card.
 *
 * ONE component for both states, not two. An enrolled programme and one the
 ...
 * a second one.
 */
export function ProgrammeCard({ progress }: { progress: ProgrammeProgress }) {
  const { programme, status, percent, moduleCount, completedCount } = progress;

  const href = `/programmes/${programme.id}`;
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-sm border border-surface-deep bg-paper-raised transition-colors duration-500 hover:border-muted-light">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className={`mt-3 line-clamp-3 ${BODY.base}`}>{programme.summary}</p>
          ...
          <>
            <div className="flex items-baseline justify-between gap-4">
              <p className={META.base}>
                {completedCount} of {moduleCount} modules
              </p>
        ...
        >
          {done ? "Review programme" : started ? "Resume" : "Start this programme"}
