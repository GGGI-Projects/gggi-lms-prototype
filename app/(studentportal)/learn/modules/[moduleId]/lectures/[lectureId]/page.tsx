...
import { curatedForLecture } from "@/lib/laws-tools";
import {
  QUIZ_LENGTH,
  blankQuestionsFor,
  blankStatus,
...
export default async function LecturePage({ params }: Params) {
  ...
  // The lecturer's own pick from this Module's related pool - most
  // lectures pick none, the same way most carry no fill-in-the-blank
  // questions (see `Lecture["relatedLawIds"/"relatedToolIds"]`).
  const related = curatedForLecture(moduleId, mod.id);
  const quizHref = `/learn/modules/${mdl.id}/lectures/${mod.id}/quiz`;
  ...
  return (
    <PageBody>
      <PageHeader
        back={{ href: `/learn/modules/${mdl.id}`, label: mdl.title }}
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-12">
        ...
        <article className="min-w-0 lg:col-span-8">
          ...
          {/* SEPARATE SECTIONS, one per library - never merged, the same
              rule the Module page follows (see the note there). Each is
              silent when empty rather than showing an empty section: a
              lecturer's curated pick is optional, most lectures have
              neither, and this is the same quiet-when-absent treatment
              fill-in-the-blank questions already get. */}
          {related.laws.length ? (
            <section className="mt-12">
              <h2 className={HEADING.card}>Related laws</h2>
          ...
          {related.tools.length ? (
            <section className="mt-12">
              <h2 className={HEADING.card}>Related tools</h2>
              <p className={`measure-wide mt-2 ${BODY.base}`}>
                Picked for this lecture specifically, from the module&rsquo;s own
                related tools.
              </p>
          ...
          <QuizCallout
            href={`/learn/modules/${mdl.id}/lectures/${mod.id}/quiz`}
            status={quiz}
            score={score}
            ...
            <Panel>
              ...
              {next ? (
                <div className="mt-3">
                  {gateCleared ? (
                    <ActionButton
                      href={`/learn/modules/${mdl.id}/lectures/${next.id}`}
                      variant="solid"
                      size="sm"
...
function LectureFooterNav({
  ...
  return (
    ...
    >
      {previous ? (
        <Link
          href={`/learn/modules/${moduleId}/lectures/${previous.id}`}
      ...
      {next ? (
        gateCleared ? (
          <Link
            href={`/learn/modules/${moduleId}/lectures/${next.id}`}
