  ...
  return (
    <PageBody>
      <PageHeader
        ...
        lead="Complete every lecture and pass every quiz - and any fill-in-the-blank questions a lecture carries - in a module and the certificate issues straight away, carrying your name, the module, the date, and a reference number that can be checked against the register."
      />
      ...
      >
        {earned.length ? (
          ...
          <EmptyState
            ...
            body="Finish every lecture, quiz and fill-in-the-blank question in any module and it issues immediately - there is no application, no fee and no waiting period."
            action={
              <ActionButton href="/learn/modules" variant="solid" size="sm">
                Browse modules
              </ActionButton>
            }
          />
              ...
              return (
                <Panel key={progress.module.id}>
                  <h3 className="font-display text-xl leading-snug tracking-tight text-ink">
                    <Link
                      href={`/learn/modules/${progress.module.id}`}
                      className="link-wipe"
                    >
                      {progress.module.title}
                    </Link>
                  ...
                  {progress.nextLecture ? (
                    <Link
                      href={`/learn/modules/${progress.module.id}/lectures/${progress.nextLecture.id}`}
